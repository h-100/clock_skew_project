...
def add_tests_to_main_page(domains, test_type, direction, browser_type, platform, test_name):
    ...
    with open('/home/hira/research/clock_skew_scripts/grace_period_experiment/flask_app/main_page/app/templates/postinclude.txt', 'r') as f:
        post = f.read()
        
    complete_html = pre + list_string + post
   
    with open('/home/hira/research/clock_skew_scripts/grace_period_experiment/flask_app/main_page/app/templates/include.html', 'w') as out:
        out.write(complete_html)
...
def run_browser_automation_web(domains, test_type, browser_type):
    ...
    if browser_type == "Chromium":
        cmd = 'chromium-browser https://time.securepki.org'
...
def main():
    ...
    parser.add_argument('--platform', help='"desktop" or "mobile"', type=str, nargs='?', default=0)
    parser.add_argument('--test_name', help='name of the experiment', type=str, nargs='?', default=0)
    ...
    if args.test_type == "notBefore":
        ...
        for i in range(int(args.domain_num)):
            ...
            domain_csr = generate_csr(domain_key, domain_name, args.platform)
        ...
        if args.platform == 'desktop':
            ...
            run_browser_automation_web(domain_names, "notAfter", args.device)
        else:
            pdb.set_trace()
            wait_until_time(base_time)
            print("Sleeping Over")
